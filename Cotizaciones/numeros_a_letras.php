...
<html>
  ...
  <body onLoad="document.forma1.cantidad.focus();">
    <form name="forma1" action="<?php echo $_SERVER['PHP_SELF']; ?>"
      method="post">
...
<?php
  function numtoletras($xcifra) {
    $xarray=array(
      0 => "Cero", 1 => "UN", "DOS", "TRES", "CUATRO", "CINCO", "SEIS", "SIETE",
    ...
    // ajusto la longitud de la cifra, para que sea divisible por centenas de
    //miles(grupos de 6)
    $xcadena="";
    for($xz=0; $xz < 3; $xz++) {
      ...
      $xi=0;
      $xlimite=6;
      //inicializo el contador de centenas xi y establezco el límite a 6 dígitos
      //en la parte entera
      ...
      while($xexit) {
        if($xi==$xlimite) // si ya llegó al límite máximo de enteros
          ...
          // comienzo con los tres primeros digitos de la cifra, comenzando por
          // la izquierda
          ...
          // ciclo para revisar centenas, decenas y unidades, en ese orden
            switch($xy) {
              case 1: // checa las centenas
                ...
                // si el grupo de tres dígitos es menor a una centena( < 99) no
                // hace nada y pasa a revisar las decenas
                else {
                  $key=(int) substr($xaux, 0, 3);
                  if(TRUE === array_key_exists($key, $xarray)) {
                  // busco si la centena es número redondo(100, 200, 300, etc..)
                    $xseek=$xarray[$key];
                    $xsub=subfijo($xaux);
                    // devuelve el subfijo correspondiente(Millón, Millones, Mil
                    // o nada)
                    if(substr($xaux, 0, 3)==100)
                      $xcadena=" ".$xcadena." CIEN ".$xsub;
                    else $xcadena=" ".$xcadena." ".$xseek." ".$xsub;
                    $xy=3;
                    // la centena fue redonda, entonces termino el ciclo del for
                    // y ya no reviso decenas ni unidades
                  }
                  else {
                  // entra aquí si la centena no fue numero redondo
              ...
              case 3: // checa las unidades
                ...
                // si la unidad es cero, ya no hace nada
                  ...
                  $xseek=$xarray[$key];
                  // obtengo directamente el valor de la unidad(de uno a nueve)
                  $xsub=subfijo($xaux);
        ...
        if(substr(trim($xcadena), -5, 5)=="ILLON") $xcadena.= " DE";
        // si la cadena obtenida termina en MILLON o BILLON, entonces le agrega
        // al final la conjuncion DE
        if(substr(trim($xcadena), -7, 7)=="ILLONES") $xcadena.= " DE";
        // si la cadena obtenida en MILLONES o BILLONES, entoncea le agrega al
        // final la conjuncion DE
        // esta línea la puedes cambiar de acuerdo a tus necesidades o a tu país
        if(trim($xaux) != "") {
          switch($xz) {
            case 0:
              if(trim(substr($XAUX, $xz * 6, 6))=="1") $xcadena.= "UN BILLON ";
              else $xcadena.= " BILLONES ";
              break;
            case 1:
              if(trim(substr($XAUX, $xz * 6, 6))=="1") $xcadena.= "UN MILLON ";
              else $xcadena.= " MILLONES ";
              break;
            case 2:
        ...
        // quito el espacio para el VEINTI, para que quede: VEINTICUATRO, etc
