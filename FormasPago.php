<!DOCTYPE html>
<html>
  <head></head>
  <body>
    <section class="contenidoSection">
      <div class="tFormasPago">
        <p>Formas de pago</p>
      </div>
    </section>
    <section class="contenidoSection">
      <div class="formaPago">
        <div class="sFormaPago">
          <div class="aBanco">
            <img src="images/banamexB.svg" class="banco1" alt="Logo Banamex">
          </div>
        </div>
        <div class="sFormaPago noCuenta">
          <p class="tFormaPago">No de cuenta<br>0451-2449-73</p>
        </div>
        <div class="sFormaPago noCuenta">
          <p class="tFormaPago">Clave interbancaria<br>0121-8000-4512-4497-33</p>
        </div>
      </div>
    </section>
    <section class="contenidoSection">
      <div class="formaPago">
        <div class="sFormaPago2">
          <div class="aBanco">
            <img src="images/BancomerLogo.svg" class="banco2"
              alt="Logo Bancomer">
          </div>
        </div>
        <div class="sFormaPago2 noCuenta2">
          <p class="tFormaPago">No de cuenta<br>4157-1802-43</p>
        </div>
        <div class="sFormaPago2 noCuenta2">
          <p class="tFormaPago">Clave interbancaria<br>0021-8002-4341-5718-04</p>
        </div>
      </div>
    </section>
    <div class="break"></div>
    <section class="contenidoSection">
      <p class="letraPago">
        Favor de mandar su ficha de deposito y su orden de compra al correo:<br>
        pagos@artefactosluminicos.com.mx
      </p>
    </section>
  </body>
</html>
